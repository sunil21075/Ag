##
## for some bizzare reason, when we separated time periods before computing
## threhsold stuff, the old buggy driver stopped working. This happened
## on Oct. 4, when I made the mistake of talking too much.
##
##
...
if(hist){
  ...
  # 5c. Iterate through future files ----------------------------------------
     ...
     for(i in 1:length(the_dir)){
         ...
         data_list_F[[i]] <- process_data_non_overlap(file, time_period="doesNotMatter")
         rm(file)
     }
