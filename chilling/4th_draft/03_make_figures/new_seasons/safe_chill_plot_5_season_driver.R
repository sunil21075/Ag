...
all_seasons_safe$location <- paste0(all_seasons_safe$lat, "_", all_seasons_safe$long)
all_seasons_safe[location=="46.28125_-119.34375"]$location <- "Richland"
all_seasons_safe[location=="48.40625_-119.53125"]$location <- "Omak"

all_seasons_safe[location=="47.40625_-120.34375"]$location <- "Wenatchee"
all_seasons_safe[location=="45.53125_-123.15625"]$location <- "Hilsboro"
all_seasons_safe[location=="44.09375_-123.34375"]$location <- "Elmira"

city_names <- c("Omak", "Wenatchee", "Richland", "Hilsboro", "Elmira")
all_seasons_safe$location <- factor(all_seasons_safe$location, levels = city_names)

starts <- c("Sept. 1", "Sept. 15", "Oct. 1", "Oct. 15", "Nov. 1", "Nov. 15")
all_seasons_safe$start <- factor(all_seasons_safe$start, levels = starts)
...
noch <- FALSE
box_width <- 1
color_ord = c("black", "grey70" , "dodgerblue", "olivedrab4", "red", "yellow") # 
time_lab = c("Hist.", "'25-'50", "'51-'75", "'76-'99")

c("Sept. 1", "Sept. 15", "Oct. 1", "Oct. 15", "Nov. 1", "Nov. 15")
...
ggsave("all_chills.png", all_boxes,
       path = "/Users/hn/Desktop/", width=35, height=15, unit="in", dpi=250)
...
dt <- within(dt, remove(year, lat, long, chill_season))
...
ggsave("all_accumulated_CP_boxes_limited_locations.png", all_boxes,
       path = "/Users/hn/Desktop/", width=35, height=15, unit="in", dpi=250)
