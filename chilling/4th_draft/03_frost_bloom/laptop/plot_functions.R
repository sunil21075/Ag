...
boxplot_frost_median_dayofyear <- function(dt, kth_day, sub_title){
  ...
  safe_b <- ggplot(data = dt, aes(x=time_period, y= median, fill=time_period)) +
            ...
            facet_grid(~ emission) +
            labs(x="", y="day count median (starting Sept. 1)") +
            the_theme + 
            scale_fill_manual(values = color_ord,
                      name = "Time\nPeriod", 
                      labels = categ_lab) + 
...
TS_frost_dayofyear <- function(data){
  ...
  acc_plot <- ggplot(data = data) +
              geom_point(aes(x = year, y = chill_dayofyear, fill = model),
                         alpha = 0.25, shape = 21) +
