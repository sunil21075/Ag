...
ensemble_map <- function(dt, scenario_name, month_col, legend_label) {
  ...
  dt %>% ggplot() +
         ...
         theme(axis.title.y = element_blank(),
               ...
               strip.text = element_text(size=12, face="bold"),
               plot.margin = margin(t=-0.5, r=0.2, b=-.5, l=0.2, unit = 'cm'),
               legend.title = element_blank()
               )
}
...
plt_width <- 10
plt_height <- 3.3
ggsave(filename = paste0("accumutaled_CP_by_Jan1_85_mean_of_models_median_of_years.png"), 
       plot=mean_of_models, 
       width=plt_width, height=plt_height, units="in", 
       dpi=600, device="png", 
       path=plot_base)

ggsave(filename = paste0("accumutaled_CP_by_Jan1_85_median_of_models_median_of_years.png"), 
...
ggsave(filename = paste0("accumutaled_CP_by_Jan1_45_mean_of_models_median_of_years.png"), 
       plot=mean_of_models, 
       width=plt_width, height=plt_height, units="in", 
       dpi=600, device="png", 
       path=plot_base)

ggsave(filename = paste0("accumutaled_CP_by_Jan1_45_median_of_models_median_of_years.png"), 
       plot=median_of_models, 
       width=plt_width, height=plt_height, units="in", 
       dpi=600, device="png", 
       path=plot_base)
