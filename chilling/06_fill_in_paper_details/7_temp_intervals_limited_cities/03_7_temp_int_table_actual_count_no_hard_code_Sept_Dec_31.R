...
in_dir <- paste0("/Users/hn/Documents/01_research_data/chilling/", 
                 "7_temp_int_limit_locs/untitled/RDS_files/several_months_together/")
source(source_path)
...
data_85 <- data.table(readRDS(paste0(in_dir, "sept_thru_dec31_modeled.rds")))
setnames(data_85, old=c("scenario"), new=c("emission"))
data_85 <- data_85 %>% filter(emission == "rcp85")
...
data_hist <- data.table(readRDS(paste0(in_dir,"sept_thru_dec31_observed.rds")))
data_hist <- data_hist %>% 
             filter(year >= 1979 & year <= 2015)%>% 
              data.table()
...
city_names <- unique(data_85_2076_2099$city)
...
out_dir <- "/Users/hn/Documents/01_research_data/chilling/7_temp_int_limit_locs/untitled/RDS_files/sept1_Dec_31_tables/"
