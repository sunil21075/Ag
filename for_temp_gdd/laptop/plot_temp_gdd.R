
rm(list=ls())
library(data.table)
library(dplyr)
library(ggplot2)
library(ggpubr)

library(Hmisc) # for negation of %in%

data_dir = "/Users/hn/Desktop/Desktop/Kirti/check_point/temp_vs_gdd/"
file_names = c("all_modeled_stat.rds", "observed_stat.rds")

modeled_stat = data.table(readRDS(paste0(data_dir, file_names[1])))
observed_stat = data.table(readRDS(paste0(data_dir, file_names[2])))
rm(data_dir, file_names)

################################################

modeled_stat$CountyGroup = as.character(modeled_stat$CountyGroup)
modeled_stat[CountyGroup == 1]$CountyGroup = 'Cooler Areas'
modeled_stat[CountyGroup == 2]$CountyGroup = 'Warmer Areas'

observed_stat$CountyGroup = as.character(observed_stat$CountyGroup)
observed_stat[CountyGroup == 1]$CountyGroup = 'Cooler Areas'
observed_stat[CountyGroup == 2]$CountyGroup = 'Warmer Areas'

################################################

modeled_historical = filter(modeled_stat, scenario=="historical")
observed_modeled = rbind(observed_stat, modeled_historical)
rm(observed_stat, modeled_historical)

# drop the years that do not belong to any climate time perios
observed_modeled <- observed_modeled[!is.na(observed_modeled$ClimateGroup),]
modeled_stat <- modeled_stat[!is.na(modeled_stat$ClimateGroup),]

# fix the order of stuff
time_periods = c("2080's", "2060's", "2040's", "Historical")
modeled_stat$ClimateGroup = factor(modeled_stat$ClimateGroup)
modeled_stat$ClimateGroup <- ordered(modeled_stat$ClimateGroup, 
                                     levels = time_periods)

observed_modeled$ClimateGroup = factor(observed_modeled$ClimateGroup)
observed_modeled$ClimateGroup <- ordered(observed_modeled$ClimateGroup, 
                                          levels = time_periods)

################################################

################################################
#######
#######                Plot Area
#######
################################################
the_theme <- theme_bw() + 
             theme(plot.margin = unit(c(t=0.1, r=0.5, b=.1, l=0.1), "cm"),
                   panel.border = element_rect(fill=NA, size=.3),
                   plot.title = element_text(hjust = 0.5),
                   plot.subtitle = element_text(hjust = 0.5),
                   panel.grid.major = element_line(size = 0.2),
                   panel.grid.minor = element_blank(),
                   panel.spacing=unit(.25,"cm"),
                   legend.position="bottom", 
                   legend.title = element_blank(),
                   legend.key.size = unit(1, "line"),
                   legend.text=element_text(size=7),
                   legend.margin=margin(t= -0.1, r = 0, b = 0, l = 0, unit = 'cm'),
                   legend.spacing.x = unit(.05, 'cm'),
                   strip.text.x = element_text(size = 10),
                   axis.ticks = element_line(color = "black", size = .2),
                   #axis.text = element_text(face = "plain", size = 2.5, color="black"),
                   axis.title.x = element_text(face = "plain", size=10, 
                   	                            margin = margin(t=4, r=0, b=0, l=0)),
                   # axis.title.x=element_blank(),
                   axis.text.x = element_text(size = 6, face = "plain", 
                   	                          color="black"),
                   axis.ticks.x = element_blank(),
                   axis.title.y = element_text(face = "plain", size = 10, 
                                               margin = margin(t=0, r=.1, b=0, l=0)),
                   axis.text.y = element_text(size = 6, face="plain", color="black")
                   # axis.title.y = element_blank()
                   )

####################################################
####################################################
####################################################

plot <- ggplot(data = modeled_stat) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, fill = factor(model)),
                   alpha = 0.6, shape = 21, size = 1) + 
        facet_grid( ~ ClimateGroup ~ CountyGroup) + 
        the_theme
ggsave(filename = "warm_cool_all_models.png", 
	   path = "/Users/hn/Desktop/", 
	   plot = plot,
       width = 10, height = 10, units = "in",
       dpi=400, 
       device = "png")

####################################################
####################################################
####################################################

plot <- ggplot(data = modeled_stat) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, fill = factor(model)),
                   alpha = 0.6, shape = 21, size = 1) + 
        the_theme
plot
ggsave(filename = "all_models_together.png", 
	   path = "/Users/hn/Desktop/", 
	   plot = plot,
       width = 10, height = 10, units = "in",
       dpi=400, 
       device = "png")

####################################################
####################################################
####################################################

plot <- ggplot(data = modeled_stat) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, fill = factor(model)),
                   alpha = 0.6, shape = 21, size = 1) + 
        facet_grid( ~ ClimateGroup) +
        the_theme
plot
ggsave(filename = "time_period.png", 
	   path = "/Users/hn/Desktop/", 
	   plot = plot,
       width = 15, height = 5, units = "in",
       dpi=400, 
       device = "png")

####################################################
#################################################### Separate the RCP45 and 85
####################################################
plot <- ggplot(data = modeled_stat) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, fill = factor(model)),
                   alpha = 0.6, shape = 21, size = 1) + 
        facet_grid( ~ ClimateGroup ~ scenario) +
        the_theme
plot
ggsave(filename = "Carbon_time_period.png", 
	   path = "/Users/hn/Desktop/", 
	   plot = plot,
       width = 15, height = 5, units = "in",
       dpi=400, 
       device = "png")

####################################################
#################################################### 
####################################################

plot <- ggplot(data = modeled_stat) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, fill = factor(model)),
                   alpha = 0.6, shape = 21, size = 1) + 
        facet_grid( ~ ClimateGroup ~ scenario ~ CountyGroup) +
        the_theme
plot
ggsave(filename = "Carbon_time_period_Warm_Cool.png", 
	   path = "/Users/hn/Desktop/", 
	   plot = plot,
       width = 15, height = 10, units = "in",
       dpi=400, 
       device = "png")
####################################################
#################################################### 
#######
#######
#######     Separate the chosen models of Cod. Moth.
#######
#######
#################################################### 
####################################################
the_six_models = c("bcc-csm1-1-m", "BNU-ESM", 
	               "CanESM2", "CNRM-CM5", 
	               "GFDL-ESM2G", "GFDL-ESM2M")
# modeled_stat$chosen = 0L
# modeled_stat$chosen[modeled_stat$model %in% the_six_models] = "chosen"
# modeled_stat$chosen[modeled_stat$chosen == "0" ] = "not_chosen"

modeled_stat_1 = modeled_stat
modeled_stat_1[(modeled_stat_1$model %nin% the_six_models)]$model = "others"
modeled_stat_1$model = factor(modeled_stat_1$model)

time_periods = c("Historical", "2040's",  "2060's", "2080's")
modeled_stat_1$ClimateGroup = factor(modeled_stat_1$ClimateGroup)
modeled_stat_1$ClimateGroup <- ordered(modeled_stat_1$ClimateGroup, 
                                     levels = time_periods)

plot <- ggplot(data = modeled_stat_1) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, 
        	           #fill = factor(model),
        	           colour = factor(model),
        	           shape = factor(model)
        	           ),
                   alpha = 1, size = 2) +
        scale_shape_manual(values = c(4,8,15,16,17,18,21,22,3,42)) +
        #scale_colour_manual(values=seq(0, 7)) +
        facet_grid( ~ ClimateGroup ~ CountyGroup) +
        the_theme
plot
ggsave(filename = "shape_color.png", 
	   path = "/Users/hn/Desktop/", 
	   plot = plot,
       width = 15, height = 8, units = "in",
       dpi=400, 
       device = "png")

###########################################################
########################################################### 
#######
#######
#######     observed comparison with modeled historical
#######
#######
########################################################### 
###########################################################
observed_modeled$scenario[observed_modeled$scenario=="historical"] = "modeled"
observed_modeled_1 = observed_modeled

# drop the model column
observed_modeled_1[, ClimateGroup:=NULL]
observed_modeled_1[, scenario:=NULL]


#********
#******** It seems observed are in F and modeled are in C. 
#******** Hence, below we convert it for 55 years!
#********
observed_modeled_1[1, "mean_cumm_dd"] = (observed_modeled_1[1, "mean_cumm_dd"] - (56*32))*5/9
observed_modeled_1[2, "mean_cumm_dd"] = (observed_modeled_1[2, "mean_cumm_dd"] - (56*32))*5/9

observed_modeled_1[1, "mean_tmean"] = observed_modeled_1[1, "mean_tmean"] * (5/9) - (5/9 * 32)
observed_modeled_1[2, "mean_tmean"] = observed_modeled_1[2, "mean_tmean"] * (5/9) - (5/9 * 32)

the_theme <- theme_bw() + 
             theme(plot.margin = unit(c(t=0.1, r=0.5, b=.1, l=0.1), "cm"),
                   panel.border = element_rect(fill=NA, size=.3),
                   plot.title = element_text(hjust = 0.5),
                   plot.subtitle = element_text(hjust = 0.5),
                   panel.grid.major = element_line(size = 0.2),
                   panel.grid.minor = element_blank(),
                   panel.spacing=unit(.25,"cm"),
                   legend.position="bottom", 
                   legend.title = element_blank(),
                   legend.key.size = unit(1, "line"),
                   legend.text=element_text(size=7),
                   legend.margin=margin(t= -0.1, r = 0, b = 0, l = 0, unit = 'cm'),
                   legend.spacing.x = unit(.05, 'cm'),
                   strip.text.x = element_text(size = 10),
                   axis.ticks = element_line(color = "black", size = .2),
                   #axis.text = element_text(face = "plain", size = 2.5, color="black"),
                   axis.title.x = element_text(face = "plain", size=12, 
                                                margin = margin(t=4, r=0, b=0, l=0)),
                   # axis.title.x=element_blank(),
                   axis.text.x = element_text(size = 8, face = "plain", 
                                              color="black"),
                   axis.ticks.x = element_blank(),
                   axis.title.y = element_text(face = "plain", size = 12, 
                                               margin = margin(t=0, r=.1, b=0, l=0)),
                   axis.text.y = element_text(size = 8, face="plain", color="black")
                   # axis.title.y = element_blank()
                   )

plot <- ggplot(data = observed_modeled_1) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, 
        	           #fill = factor(model),
        	           colour = factor(model),
        	           shape = factor(model)
        	           ),
                   alpha = 1, size = 2) +
        scale_shape_manual(values = seq(1:20)) +
        #scale_colour_manual(values=seq(0, 7)) +
        facet_grid(~ CountyGroup) +
        the_theme
# plot
ggsave(filename = "observed_modeled_1.png", 
	   path = "/Users/hn/Desktop/", 
	   plot = plot,
       width = 6, height = 6, units = "in",
       dpi=400, 
       device = "png")

observed_modeled_2 = observed_modeled_1
observed_modeled_2[(observed_modeled_2$model %nin% c("observed"))]$model = "modeled"

plot <- ggplot(data = observed_modeled_2) +
        geom_point(aes(x = mean_cumm_dd, y = mean_tmean, 
                     #fill = factor(model),
                     colour = factor(model),
                     shape = factor(model)
                     ),
                   alpha = 1, size = 2) +
        scale_shape_manual(values = seq(1:20)) +
        #scale_colour_manual(values=seq(0, 7)) +
        facet_grid(~ CountyGroup) +
        the_theme
# plot
ggsave(filename = "observed_modeled_2.png", 
     path = "/Users/hn/Desktop/", 
     plot = plot,
       width = 6, height = 6, units = "in",
       dpi=400, 
       device = "png")


