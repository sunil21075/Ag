...
RDS_convert <- function(file_name, file_path){
  df <- read_binary_(file_name, file_path, TRUE)
  saveRDS(df, paste0("RDS/historical/", file_name,".rds"))
}

df <- tibble(file_name = list.files("aeolus/data/historical/"),
       file_path = list.files("aeolus/data/historical/", full.names = TRUE))
