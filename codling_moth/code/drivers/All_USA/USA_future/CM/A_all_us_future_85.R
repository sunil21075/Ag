...
write_path = "/data/hydro/users/Hossein/codling_moth_new/all_USA/processed/future_CM/"
...
for(version in c('rcp85')) {
  ...
  for( file in files) {
    ...
    if(category == "historical") {
      temp$ClimateGroup[temp$year >= start_year_h & temp$year <= end_year_h] <- "Historical"
      temp_data <- rbind(temp_data, temp[temp$year >= start_year_h & temp$year <= end_year_h, ])
    }
    else {
