...
for (model in models){
	...
	all_slopes <- setNames(data.table(matrix(nrow = 0, ncol = 10)), 
		                   c("location", climate_scenarios, "month", "CountyGroup"))
	...
	for (curr_month in months){
		...
		for (curr_location in locations){
			...
			for (climate_scenario in climate_scenarios){
			    ...
			    curr_slopes[curr_slopes$location == curr_location, "CountyGroup"] = data$CountyGroup
			}
		}
