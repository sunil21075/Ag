...
########################## Functions
##########################
dw_pupa_1 <- function(x, shape=params[9, "shape"], scale=params[9, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}

dw_pupa_2 <- function(x, shape=params[10, "shape"], scale=params[10, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}

dw_pupa_3 <- function(x, shape=params[11, "shape"], scale=params[11, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}

dw_pupa_4 <- function(x, shape=params[12, "shape"], scale=params[12, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}
######################### S H I F T 10 %
dw_pupa_1_shift <- function(x, shape=params_shift[9, "shape"], scale=params_shift[9, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}

dw_pupa_2_shift <- function(x, shape=params_shift[10, "shape"], scale=params_shift[10, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}

dw_pupa_3_shift <- function(x, shape=params_shift[11, "shape"], scale=params_shift[11, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}

dw_pupa_4_shift <- function(x, shape=params_shift[12, "shape"], scale=params_shift[12, "scale"]){
  dweibull(x, shape=shape, scale=scale)
}
########################## 
########################## Functions
...
pupa_density =ggplot(data.frame(x=x_limits), aes(x=x)) + the_theme + 
			  ...
			  geom_path(stat="function", fun=dw_pupa_4, aes(colour="red"), linetype=1)+
			  geom_path(stat="function", fun=dw_pupa_1_shift, aes(colour="grey70"), linetype=2)+
			  geom_path(stat="function", fun=dw_pupa_2_shift, aes(colour="dodgerblue"), linetype=2)+
