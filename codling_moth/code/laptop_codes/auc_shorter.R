...
for (file in file_list){ 
    ...
    if (pop_type=="rel"){
      data_total <- data[variable == "RelLarvaPop"]
      data_escap <- data[variable == "RelNonDiap" ]
    } else {
      data_total <- data[variable == "AbsLarvaPop"]
      data_escap <- data[variable == "AbsNonDiap" ]
    }
    ...
    for (time_p in time_periods){
        ...
        if (time_p=="Historical"){
            data <- data[data$ClimateGroup == "Historical"]
        } else if (time_p=="2040's"){
            data <- data[data$ClimateGroup == "2040's"]
        } else if (time_p=="2060's"){
            data <- data[data$ClimateGroup == "2060's"]
        } else if (time_p=="2080's"){
            data <- data[data$ClimateGroup == "2080's"]
        }
    ...
    out_name = paste0(pop_type, "_",model_type)
}