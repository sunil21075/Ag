...
the_theme <- theme_bw() + 
               theme(plot.margin = unit(c(t=1, r=1, b=.5, l=.5), "cm"),
                     ...
                     axis.text.x = element_text(size = 18, face = "plain", color="black"),
...
the_theme <- theme_bw() + 
             theme(plot.margin = unit(c(t=1, r=1, b=.5, l=.5), "cm"),
                   ...
                   axis.ticks.x = element_blank(),
                   axis.title.y = element_text(face = "plain", size = 28, 
                                               margin = margin(t=0, r=20, b=0, l=0)),
                   axis.text.y = element_text(size = 28, face="plain", color="black")
                   # axis.title.y = element_blank()
                  )

facet45_85 <- ggplot(data = data_back, aes(x=variable, y=value), group = variable) + 
                geom_boxplot(outlier.size=-.15, notch=FALSE, width=.6, lwd=.55, aes(fill=ClimateGroup), 
...
the_theme <- theme_bw() + 
               theme(plot.title = element_text(size = 26, face = "bold"), 
                     ...
                     axis.ticks.x = element_blank(),
                     axis.title.y = element_text(face = "plain", size = 26, 
