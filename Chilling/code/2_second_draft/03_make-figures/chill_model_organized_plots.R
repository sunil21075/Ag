...
summary_comp <- data.table(readRDS(paste0(write_dir_, "summary_comp.rds")))
...
thresh_new_plot <- function(data, percentile){
  ...
  the_plot <- ggplot(data = data) +
              ...
              scale_y_continuous(breaks=c(0, 50, 100, 122, 137, 150, 200, 250, 300)) + 
              ylab("Median days") +
              xlab("Year") +
              ggtitle(label = lab) +
              theme_bw() + 
...
accum_plot <- function(data, y_name, due){
  ...
  acc_plot <- ggplot(data = summary_comp_loc_medians) +
              geom_point(aes(x = year, y = y, fill = scenario),
                         alpha = 0.25, shape = 21) +
              geom_smooth(aes(x = year, y = y, color = scenario),
...
accum_hist_plot <- function(data, y_name, due){
  ...
  hist_plt <- ggplot(data = summary_comp_hist) +
