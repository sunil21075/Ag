...
if(hist){
  ...
  for(i in 1:length(the_dir)){
    ...
    data_list_2040[[i]] <-  file %>%
                            ...
                            group_by(Chill_season) %>%
                            # Mutate output is the row index of the first time 
                            # where it meets threshold
                            ...
                            mutate(thresh_50 = detect_index(.x = Cume_portions,
                                   ...
                                   thresh_75 = detect_index(.x = Cume_portions,
                                                            .f = chill_thresh,
                                                            threshold = 75),
                             # Below we set the row where, ex. month = 1 (jan) and 
                             # day = 1 to be
                             # equal to the value of Cume_portions. 
                             # This creates one non-NA row
                             # for the whole column. Then in summarise we remove 
                             # all NAs, which
                             # collapses the column to a single value -- perfect 
                             # for a one-line
                             # summary per season.
                                   sum_J1 =  case_when(
                                     Month == 1 & Day == 1 ~ Cume_portions),
