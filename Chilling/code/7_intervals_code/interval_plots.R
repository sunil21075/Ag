...
############ modeled
############
data <- data.table(readRDS(paste0(input_dir, "sept_through_dec_modeled.rds")))
data <- data %>% 
          mutate(temp_cat=cut(Temp, breaks=iof_breaks)) %>% 
          group_by(Chill_season, Year, model, Month, scenario, temp_cat, CountyGroup) %>% 
          summarise(no_hours = n())                      
...
ggsave(plot = sep_Dec_plot,
       filename = paste0("sep_through_Dec", ".png"),
       path = "/Users/hn/Desktop/",
       device = "png",
       height = 5, width = 8, units = "in", dpi=400)
...
############ observed
############
data <- data.table(readRDS(paste0(input_dir, "sept_through_dec_observed.rds")))
data <- data %>% 
          mutate(temp_cat=cut(Temp, breaks=iof_breaks)) %>% 
          group_by(Chill_season, Year, model, Month, scenario, temp_cat, CountyGroup) %>% 
          summarise(no_hours = n())                      
...
sep_Dec_plot <- plot_intervals(data, month_name="Sept. through Dec.")
ggsave(plot = sep_Dec_plot,
       filename = paste0("sep_through_Dec_observed.png"),
       path = "/Users/hn/Desktop/",
       device = "png",
       height = 5, width = 8, units = "in", dpi=400)
