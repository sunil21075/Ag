...
# save_plot("figures/daily_exceedance_probability_dry.pdf", 
# p_prob, base_aspect_ratio = 2, scale = 1, limitsize = FALSE)
...
p_prob_monthly <- plot_grid(p_monthly_exceedance_val,p_octmar_exceedance_val, 
                            nrow = 1, align = "vh", 
                            rel_widths = c(3.25, 1), 
                            axis = 'b')

p_all <- plot_grid(p_prob_daily, p_prob_7day, p_prob_monthly, 
                   ncol = 1, align = "vh")

# save_plot("figures/monthly_exceedance_probability_wet.pdf", 
# p_prob, base_aspect_ratio = 2, scale = 1, limitsize = FALSE)

save_plot("figures/exceedance_probability_dry.pdf", p_all, 
          base_aspect_ratio = .5, scale = 3.5, limitsize = FALSE)
...
# 
df_cum_precip <- df %>% 
                 mutate(water_date = time_stamp %m+% months(3),
                        water_week = week(water_date),
                        water_year = year(water_date),
                        climate_proj = ifelse(is.na(climate_proj), 
                                              "historical", 
                                               as.character(climate_proj))) %>%
                 filter(precip > 0, month >= 10 | month <= 3) %>%
...
### DRY DAYS ####
p_monthly_dry <- df %>% 
                 group_by(month, year, group, model, climate_proj) %>%
...
p_octmar_dry <- df %>% 
                mutate(water_year = year(time_stamp + month(3))) %>%
...
p_dry_dry <- plot_grid(p_monthly_dry, p_octmar_dry, 
                       rel_widths = c(5.5, 1), 
                       align = 'vh', nrow = 1, 
                       axis = "b")
...
save_plot("figures/dry_days.pdf", p_dry, 
           base_aspect_ratio = 1.5, scale = 2, limitsize = FALSE)
...
save_plot("figures/precip_density_dry_zoom.pdf", p, 
           base_aspect_ratio = 1.618, scale = 2)
