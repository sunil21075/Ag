...
header <- dashboardHeader(
  title = tags$div(tags$img(src='WSU_logo.png', height = 30),
                   "CAHNRS"))
...
body <- dashboardBody(
      ...
      tabPanel("Map",
               div(class = "outer",
                 ...
                 absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
                               ...
                               h3(tags$b("Explorer")),
                               radioButtons("precip_map_climate_proj", label = "Climate Projection", 
                                            choices = list("RCP 4.5" = "rcp45", "RCP 8.5" = "rcp85"),
                                            selected = "rcp85"),
                               radioButtons("precip_map_exceedance", label = "Exceedance Value", 
                                            choices = list("20th Percentile" = "prob_80", 
                                                           "10th Percentile" = "prob_90",
                                                           "5th Percentile" = "prob_95"),
                                            selected = "prob_80"),
                               radioButtons("precip_map_climate_group", label = "Climate Group", 
                                            choices = list("2040's" = "2040s", "2060's" = "2060s", "2080's" = "2080s"),
                                            selected = "2040s"),
                               radioButtons("precip_map_time_scale", label = "Time Scale", 
  ...
  bsModal(title = "Precipitation Summary", id = "precip_graphs", trigger = NULL, size = "large",
          fluidPage(
            fluidRow(column(2, radioButtons("precip_plot_climate_proj", label = h4("Climate Projection"), 
                                            choices = list("RCP 4.5" = "rcp45", "RCP 8.5" = "rcp85"),
                                            selected = "rcp85"),
  ...
  bsModal(title = "Surface Flow Summary", id = "surface_graphs", trigger = NULL, size = "large",
          fluidPage(
            fluidRow(column(2, radioButtons("surface_plot_climate_proj", label = h3("Climate Projection"), 
                                            choices = list("A1B" = "A1B", "B1" = "B1"), 
                                            selected = "A1B")),
