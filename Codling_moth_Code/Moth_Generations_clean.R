...
CodlingMothPercentPopulation = function(CodMothParams, metdata_data.table) {
  ...
  for (i in 5:8) {
    columnname = paste("Perc", CodMothParams[i,1], "Gen", CodMothParams[i,2],sep="")
    columnnumber = which( colnames(allrelnum)==columnname )
    allrelnum$PercLarva[allrelnum$Cum_dd_F > CodMothParams [i,5] & allrelnum$Cum_dd_F <= CodMothParams [i,6]] = 
             allrelnum[allrelnum$Cum_dd_F > CodMothParams [i,5] & allrelnum$Cum_dd_F <= CodMothParams [i,6], columnnumber]
  }  
  for (i in 9:12) {
    columnname = paste("Perc",CodMothParams[i,1], "Gen",CodMothParams[i,2],sep="")
    columnnumber = which( colnames(allrelnum)==columnname )
    allrelnum$PercPupa[allrelnum$Cum_dd_F > CodMothParams [i,5] & allrelnum$Cum_dd_F <= CodMothParams [i,6]] = 
               allrelnum[allrelnum$Cum_dd_F > CodMothParams [i,5] & allrelnum$Cum_dd_F <= CodMothParams [i,6], columnnumber]
  } 
  for (i in 13:16) {
    columnname = paste("Perc",CodMothParams[i,1], "Gen",CodMothParams[i,2],sep="")
    columnnumber = which( colnames(allrelnum)==columnname )
    allrelnum$PercAdult[allrelnum$Cum_dd_F > CodMothParams [i,5] & allrelnum$Cum_dd_F <= CodMothParams [i,6]] = 
             allrelnum[allrelnum$Cum_dd_F > CodMothParams [i,5] & allrelnum$Cum_dd_F <= CodMothParams [i,6],columnnumber]
...
# End of CodlingMothRelPopulation which started at line 70
...
add_dd_cumudd  =  function(metdata_data.table, lower, upper) {
  ...
  tempdata$heat[tempdata$tmin < lower & tempdata$tmax > lower & tempdata$tmax <= upper] = 
          (((tempdata$aveminlt[tempdata$tmin < lower & tempdata$tmax > lower & tempdata$tmax <= upper] * 
               (pihalf - tempdata$theta1[tempdata$tmin < lower & tempdata$tmax > lower & tempdata$tmax <= upper])) + 
              (tempdata$alpha1[tempdata$tmin < lower & tempdata$tmax > lower & tempdata$tmax <= upper] * 
                 cos(tempdata$theta1[tempdata$tmin < lower & tempdata$tmax > lower & tempdata$tmax <= upper]))) / pi)
  
  # case 5 tmin>=lower threshold & tmin<upper & tmax>upper threshold
  tempdata$heat[tempdata$tmin >= lower & tempdata$tmin < upper & tempdata$tmax > upper] = 
    (((tempdata$aveminlt[tempdata$tmin >= lower & tempdata$tmin < upper & tempdata$tmax > upper] * 
         (tempdata$theta2[tempdata$tmin >= lower & tempdata$tmin < upper & tempdata$tmax > upper] + pihalf)) + 
        ((upper - lower) * (pihalf - tempdata$theta2[tempdata$tmin >= lower & tempdata$tmin < upper & tempdata$tmax > upper])) - 
        (tempdata$alpha1[tempdata$tmin >= lower & tempdata$tmin < upper & tempdata$tmax > upper] * 
           cos(tempdata$theta2[tempdata$tmin >= lower & tempdata$tmin < upper & tempdata$tmax > upper]))) / pi)
 
  # case 6 tmin<lower threshold & tmax>upper threshold
  tempdata$heat[tempdata$tmin < lower & tempdata$tmax > upper] = 
    (((tempdata$aveminlt[tempdata$tmin < lower & tempdata$tmax > upper] * 
         (tempdata$theta2[tempdata$tmin < lower & tempdata$tmax > upper] - 
            tempdata$theta1[tempdata$tmin < lower & tempdata$tmax > upper])) + 
        (tempdata$alpha1[tempdata$tmin < lower & tempdata$tmax > upper] * 
           (cos(tempdata$theta1[tempdata$tmin < lower & tempdata$tmax > upper]) - 
              cos(tempdata$theta2[tempdata$tmin < lower & tempdata$tmax > upper]))) + 
        ((upper - lower) * (pihalf - tempdata$theta2[tempdata$tmin < lower & tempdata$tmax > upper]))) / pi)
...
# End of add_dd_cumudd which started at line 100
...
# End of create_ymdvalues which started at line 156

#  function to read binary data
readbinarydata_addmdy = function(filename, Nrecords, Nofvariables, ymd, ind) {
  # create a connection (what the hell does it mean?) rb = Open for reading in binary mode
  fileCon = file(filename, open = "rb")
...
# End of readbinarydata_addmdy which started at 190
########################################################################
...
calcPopulation  =  function(filename, input_folder, output_folder)
{
  ...
  ## indices just denote the fact that, given n variables, a new day's data starts every nth read (what does this mean?)
  ...
  Nofvariables = 4 # number of varaibles or column in the forcing data file (why hard code?)
  ...
  ## read meta data and add year month day variables
  ...
  colnames(toprint) = c("tmax","tmin","DailyDD","CumDDinC","CumDDinF",
...
# End of calcPopulation which started at line 213
...
# End of mothPopulation which started at line 708
...
# End of getRelPopulation which started at line 777
...
prepareData = function(filename, input_folder, start_year, end_year) {
  ...
  data  =  cbind(metdata_data.table$tmax, 
                 metdata_data.table$tmin, 
                 metdata_data.table$dd, 
                 metdata_data.table$Cum_dd, 
                 metdata_data.table$Cum_dd_F, 
...
# End of prepareData which started at line 826
...
# End of prepareData_1 which started at line 902
########################################
