...
for (time_p in time_periods){
  for (target_fip in local_fips){
    for(model_n in model_names){
      ...
      DT <- produce_dt_for_pie_Q2(analog_dt=analog_dat, novel_dt=novel_dat, tgt_fip=target_fip, 
      ...
      assign(x = gsub("-", "_", model_n), value = {plot_the_pie_Q2(DT, titlem)})
    }
...
master_path <- paste0(data_dir, "/ss_total/")
