...
for (time_p in time_periods){
  for (target_fip in local_fips){
    for(model_n in model_names){
      ...
      DT <- produce_dt_for_pie_all_possible(analog_dt=analog_dat, novel_dt=novel_dat, tgt_fip=target_fip, 
      ...
      assign(x = gsub("-", "_", model_n), value = {plot_the_pie_all_possible(DT, titlem)})
    }
...
master_path <- paste0(data_dir, "/ss_all-possible/")
if (dir.exists(master_path) == F) { dir.create(path = master_path, recursive = T)}

ggsave(paste0("don_2026_2050.png"), plot_2026_2050, 
       path=master_path, device="png",
       dpi=300, width=40, height=100, unit="in", limitsize = FALSE)

ggsave(paste0("don_2051_2075.png"), plot_2051_2075, 
       path=master_path, device="png",
       dpi=300, width=40, height=100, unit="in", limitsize = FALSE)


ggsave(paste0("don_2076_2095.png"), plot_2076_2095, 
       path=master_path, device="png",
       dpi=300, width=40, height=100, unit="in", limitsize = FALSE)
