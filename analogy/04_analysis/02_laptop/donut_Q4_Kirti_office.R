...
hist_counties_with_more_10 <- "hist_counties_with_more_10.csv"
...
hist_counties_with_more_10 <- read.csv(paste0(param_dir, hist_counties_with_more_10)) %>%
                              data.table()
...
for (sigma_bd in sigma_bds){
  ...
  for (target_fip in local_fips){
    for(model_n in model_names){
      ...
      #############################################################################
      #
      #   Filter out counties with less than 10 girids in them.
      #
      
      analog_dat_F1 <- analog_dat_F1 %>% 
                       filter(analog_NNs_county %in% hist_counties_with_more_10$fips)
    ...
    assign(x = paste0("plot_", target_fip) , 
           value={ggarrange(plotlist = list(map_bcc_csm1_1_m_F1, map_bcc_csm1_1_m_F2, map_bcc_csm1_1_m_F3,
                            ...
                            heights=c(2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1), 
