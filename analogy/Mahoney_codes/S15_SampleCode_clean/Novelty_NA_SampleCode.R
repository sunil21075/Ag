...
## Basic Code for calculating and mapping climatic novelty 
## using the sigma dissimilarity metric. 
# NB:  For simplicity, 
#      this code calculates dissimilarity based on the interannual 
#      climatic variability of the closest CRU TS3.23 weather station. 
#      The results in the manuscript are the inverse-distance-weigthed 
#      average of novelty calculated from the nearest four weather stations, 
#      which are specifed in the grid4nn_NAnaec8.csv file     

# Colin Mahony
# Centre for Forest Conservation Genetics and 
# Department of Forest and Conservation Sciences, 
# University of British Columbia, 3041-2424 Main Mall, 
...
# "Novelty_NA_LocalStPCA_InputData_Feb2016.R", except for the 
# cru surrogates, which were created in the Oct2015 version. 
...
# 2071-2100 climate normals (RCP4.5 ensemble mean projection) 
# for all land cells of the DEM. 
...
for(j in sort(unique(proxy))){
  ...
  Cj.sd    <- apply(Cj,MARGIN=2, FUN = sd, na.rm = T)
  A.prime  <- sweep(A, MARGIN=2, STATS = Cj.sd, FUN = `/`) # standardize the reference ICV
  Bj.prime <- sweep(Bj,MARGIN=2, STATS = Cj.sd, FUN = `/`) # standardize the analog pool
  
  # standardize the projected future conditions 
  # of grid cells represented by ICV proxy j
  Cj.prime <- sweep(Cj, MARGIN=2, Cj.sd, FUN =`/`)
  ...
  Zj.sd <- apply(Zj, MARGIN=2, FUN = sd, na.rm = T)

  # standardize the analog pool   
  X.prime <- sweep(X, MARGIN=2, Zj.sd, FUN = `/`)
  ...
  NN.dist <- as.vector(get.knnx(data=X.prime[, 1:PCs], 
                                query = Yj.prime[, 1:PCs], k=1, algorithm="brute")[[2]])
