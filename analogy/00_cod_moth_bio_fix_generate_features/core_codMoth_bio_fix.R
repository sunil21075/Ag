apply_bio_fix_to_CMPOP <- function(bad_CMPOP, biofix_param){
  ...
  ### Fix the goddamn DD for non-local grids.
  # 1- separate local from non_locals
  good_CMPOP_local <- bad_CMPOP %>% 
                     filter(!(location %in% biofix_param$location))

  bad_CMPOP_non_local <- bad_CMPOP %>%
                         filter(location %in% biofix_param$location)

  # 2- Fix the fucking non_local daily_DD ($DailyDD)
       ...
       # f) fucking Done
  bad_CMPOP_non_local <- data.table(as.data.frame(bad_CMPOP_non_local))
  bad_CMPOP_non_local <- merge(bad_CMPOP_non_local, biofix_param, by="location", all.x=T)
  bad_CMPOP_non_local$biofix[bad_CMPOP_non_local$CumDDinC < bad_CMPOP_non_local$biofix] <- 0
  bad_CMPOP_non_local$biofix[bad_CMPOP_non_local$biofix != 0] <- bad_CMPOP_non_local$DailyDD[bad_CMPOP_non_local$biofix != 0]
  ...
  bad_CMPOP_non_local$biofix[bad_CMPOP_non_local$biofix<0.001] <- 0
  ...
  bad_CMPOP_non_local[, CumDDinC := cumsum(CumDDinC), by = list(location, year)]


  bad_CMPOP <- rbind(good_CMPOP_local, bad_CMPOP_non_local)
  bad_CMPOP$CumDDinF = bad_CMPOP$CumDDinC * 1.8
  
  rm(good_CMPOP_local, bad_CMPOP_non_local)
  return (bad_CMPOP)
}
...
prepareData_CMPOP_biofix <- function(bad_CMPOP, cod_param, biofix_param,
                                     cod_moth_param_name,
                                     scale_shift=0){
  
  bad_CMPOP <- apply_bio_fix_to_CMPOP(bad_CMPOP, biofix_param)
...
########################################################################################################
########################################################################################################
prepareData_CM <- function(filename, input_folder, param_dir, cod_moth_param_name, scale_shift=0, start_year, end_year, lower, upper){
...
######################################################################################
##############                                                                                        
############## Phase 3: Using combined CM and CMPOP files to produce stuff, 
##############          like vertdd, bloom, etc. 
##############
#####################################################################################
#######################                                            ##################
#######################                vertdd function             ##################
...
generate_vertdd <- function(input_dir, 
  ...
  data = data[, vert_Cum_dd := cumsum(vertdd), 
                by=list(latitude, longitude, ClimateScenario, ClimateGroup, year)]
...
mean_emerge_time_bio_low_hi <- function(data_dt){
  data_dt_low <- data_dt %>% filter(elevation <= 400) %>% data.table()
  data_dt_hi <- data_dt %>% filter(elevation > 400) %>% data.table()

  data_dt_low <- mean_emerge_time_bio_low(data_dt_low)
  data_dt_hi <- mean_emerge_time_bio_high(data_dt_hi)

  data_dt <- rbind(data_dt_low, data_dt_hi)
  data_dt <- data.table(data_dt)
  return(data_dt)
}

mean_emerge_time_bio_high <- function(data_dt){
  data_dt$biofix <- 1755.599 - 
                    (66.777 * data_dt$lat) + 
                    (0.676 * (data_dt$lat^2)) -
                    (0.0347 * data_dt$elevation)
  return(data_dt)
}

mean_emerge_time_bio_low <- function(data_dt){
  data_dt$biofix <- 1923.387 - 
                    (73.806 * data_dt$lat) + 
                    (0.745 * (data_dt$lat^2))
  return(data_dt)
}
