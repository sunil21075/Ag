...
##################
## these files were created in a separate script 
# "Novelty_NA_LocalStPCA_InputData_Feb2016.R", except for 
# the cru surrogates, which were created in the Oct2015 version. 
...
# ICV proxy data. Linearly detrended 1951-1990 annual time series 
# at selected CRU TS3.23 climate stations. these time series are 
# used as proxies for local interannual climate variability ("ICV proxies")
...
# initiate the data frame to store the projected 
# sigma dissimilarity of best analogs for each grid cell. 
...
for(j in sort(unique(proxy))){ 
  ...
  ## Step 1: express climate data as standardized 
  # anomalies of reference period (1951-1990) ICV at ICV proxy j. 
  
  # standard deviation of 1951-1990 interannual variability 
  # in each climate variable, ignoring missing years
  ...
  # standardize the projected future conditions of grid cells represented by ICV proxy j
  Cj.prime <- sweep(Cj,MARGIN=2, Cj.sd, `/`) 
  
  ## Step 2: Extract the principal components (PCs) of the 
  # reference period ICV and project all data onto these PCs
  
  # Principal components analysis. The !is.na(apply(...)) term is there simply 
  # to select all years with complete observations in all variables. 
  PCA <- prcomp(Cj.prime[!is.na(apply(Cj.prime,1,mean)),])   
  
  # find the number of PCs to retain using the 
  # PC truncation rule of eigenvector stdev > the truncation threshold
  ...
  # Euclidean nearest neighbour distance in the 
  # z-standardized PCs of interannual climatic variability, 
  # i.e. the Mahalanobian nearest neighbour. 
  NN.dist <- as.vector(get.knnx(data=X.prime[,1:PCs], 
                                query=Yj.prime[,1:PCs], k=1, 
                                algorithm="brute")[[2]]) 
  
  # percentile of the nearest neighbour distance 
  # on the chi distribution with degrees of freedom 
  # equaling the dimensionality of the distance measurement (PCs)
  NN.chi <- pchi(NN.dist,PCs) 

  # values of the chi percentiles on a standard half-normal 
  # distribution (chi distribution with one degree of freedom)
...
# the digital elevation model (DEM) used to generate 
# the input data. This is used as a template 
# raster for mapping the results. 
dem <- raster("dem8.tif") 
...
png(filename=paste("NoveltyMap.png",sep="_"), 
    type="cairo", units="in", width=9, height=8, pointsize=16, res=800)
par(mar=c(0,0,0,0))
...
text(rep(xr,3), c(yb, mean(c(yt, yb)), yt-100000), 
     ...
     bquote(.(breakseq[2])*sigma), bquote(.(breakseq[3])*sigma)), 
     as.expression), pos=4, cex=1, font=1)  
...
#####################
####### Code for preparing ClimateNA files for 
####### use as an alternate B matrix for other time periods, 
####### other model projections, other climate variables etc.
#####################

# Climate NA can be obtained here: 
...
# download projected normals for North America ("NA") 
# in a North American equidistant conic ("naec") projection at 8-km resolution. 
...
#central year of the normal period, e.g., 2085 indicates the 2071-2100 normal period.
proj.year <- 2085 
...
for(i in grep("PPT", names(X.grid.proj))){
  # set zero values to one, to facilitate log-transformation
  X.grid.proj[which(X.grid.proj[,i]==0),i] <- 1
}  
...
# This file can be used as the B matrix to 
# calculate novelty for other time periods 
# and/or other model projections. 
