...
if (time_type == "future"){
	input_dir = paste0(write_dir, "local/data_bases/")
    write_dir = paste0(write_dir, "local/data_bases/")
    version = args[2]
    file_name = paste0("Rel_prep_4_diap_CMPOP_", version, ".rds")
...
result = gen_diap_map1_4_analog_Rel(input_dir, file_name, param_dir,
	                                time_type,
	                                CodMothParams_name = "CodlingMothparameters.txt", 
	                                location_group_name = "LocationGroups.csv")

saveRDS(result, paste0(write_dir, "diapause_map1_clean_", version, ".rds"))
