...
for (dt_type in in_dir_ext){ # precip or rain or runoff?
  ...
  for (timeP_ty in 1:1){ # annual or chunk or wtr_yr?
    ...
    plot_dir <- paste0(in_dir, "new_2_", dt_type, "/monthly", "_", dt_type, "/perc_diffs/")
    ...
    for (clust_g in cluster_types){
      ...
      # assign(x = paste0("unbias_", unlist(strsplit(clust_g, " "))[1], "_mag_diff_45"),
      #        value = {box_trend_monthly_cum(dt=curr_unbias_diff_45, p_type="box",
      #                                       y_lab=mag_y_lab, tgt_col = "diff") + 
      #                 ggtitle(box_title)
      #                 })

      # assign(x = paste0("unbias_", unlist(strsplit(clust_g, " "))[1], "_mag_diff_85"),
      ...
      # assign(x = paste0("bias_", unlist(strsplit(clust_g, " "))[1], "_mag_diff_45"),
      #        value = {box_trend_monthly_cum(dt=curr_bias_diff_45, p_type="box",
      #                                       y_lab=mag_y_lab, tgt_col = "diff") + 
      #                 ggtitle(box_title)
      #                })

      # assign(x = paste0("bias_", unlist(strsplit(clust_g, " "))[1], "_mag_diff_85"),
      ...
      assign(x = paste0("bias_", unlist(strsplit(clust_g, " "))[1], "_perc_diff_45"),
             value = {box_trend_monthly_cum(dt = curr_bias_diff_45, p_type="box",
                                            y_lab = "differences (%)",
                                            tgt_col = "perc_diff") + 
                      ggtitle(box_title)
    ...
    biased_RCP45 <- ggarrange(plotlist = list(bias_least_perc_diff_45,
                                              bias_lesser_perc_diff_45,
                                              bias_less_perc_diff_45,
                                              bias_most_perc_diff_45
                                              ),
                              ncol = 1, nrow = 4,
                              common.legend = TRUE, legend="bottom")

    biased_RCP85 <- ggarrange(plotlist = list(bias_least_perc_diff_85,
                                              bias_lesser_perc_diff_85,
                                              bias_less_perc_diff_85,
                                              bias_most_perc_diff_85
                                              ),
                              ncol = 1, nrow = 4,
                              common.legend = TRUE, legend="bottom")
