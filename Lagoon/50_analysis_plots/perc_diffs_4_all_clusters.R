...
for (dt_type in in_dir_ext){ # precip or rain or runoff?
  ...
  for (timeP_ty in 1:3){ # annual or chunk or wtr_yr?
    ...
    bias_perc_diff_85 <- ann_wtrYr_chunk_cum_box_cluster_x(dt = bias_diff_85,
                                                           y_lab = "differences (%)",
                                                           tgt_col = "perc_diff",
                                                           ttl = box_title, 
                                                           subttl = box_subtitle)
    ...
    ggsave(filename = paste0(timeP_ty_middN[timeP_ty], "_biased_diffs_RCP85.png"),
           plot = bias_perc_diff_85, 
           width = 6, height = 2.5, units = "in", 
           dpi = 400, device = "png",
           path = plot_dir)
