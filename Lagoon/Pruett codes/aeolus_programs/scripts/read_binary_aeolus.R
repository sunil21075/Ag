...
read_binary <- function(file_name, file_path, model_name, climate_proj){
  ...
  if (model_name == "historical") {
    df_hist <- readbinarydata_addmdy(full_path, ymd_file) %>% 
      as_tibble() %>%
      mutate(group = "hist",
             model = model_name,
             climate_proj = NA)
    ...
    df_futr <- readbinarydata_addmdy(full_path, ymd_file) %>%
      as_tibble()
