...
bias_dir_ext <- "/02_med_diff_med_obs/model_killed/"
unbias_dir_ext <- "/02_med_diff_med_no_bias/model_killed/"
...
for (dt_type in in_dir_ext){ # precip or rain or runoff?
  ...
  for (timeP_ty in 1:1){ # annual or chunk or wtr_yr?
    ...
    bias_diff <- bias_diff %>% filter(month %in% c(11, 12))%>% data.table()
    ...
    ##################################################################
    #
    # KILL effect of model?
    #
    ##################################################################
    AVs <- AVs %>% filter(time_period != "2006-2025") %>% data.table()
    AVs <- AVs %>%
           group_by(model, time_period, emission, cluster, month) %>%
    ...
    for (clust_g in cluster_types){
      ...
      ######### Actual value plots
      #########
      AV_box_85 <- Nov_Dec_cum_box(dt = curr_AVs_85, y_lab = AV_y_lab, tgt_col = "AV")
      AV_box_85 <- AV_box_85 + ggtitle(paste0("monthy cum. ", dt_type, "."))

      AV_box_45 <- Nov_Dec_cum_box(dt = curr_AVs_45, y_lab = AV_y_lab, tgt_col = "AV")
      AV_box_45 <- AV_box_45 + ggtitle(paste0("monthy cum. ", dt_type, "."))
      #########
      ...
      unbias_mag_diff_45 <- Nov_Dec_Diffs(dt=curr_unbias_diff_45,
                                          y_lab=mag_y_lab,
                                          tgt_col = "diff",
                                          ttl=box_title, 
                                          subttl=box_subtitle)
      ...
      unbias_mag_diff_85 <- Nov_Dec_Diffs(dt=curr_unbias_diff_85,
                                          y_lab=mag_y_lab,
                                          tgt_col = "diff",
                                          ttl=box_title, 
                                          subttl=box_subtitle)
      ...
      box_subtitle <- "for each model median is\ntaken over years, separately"
      bias_mag_diff_45 <- Nov_Dec_Diffs(dt=curr_bias_diff_45,
                                        y_lab=mag_y_lab,
                                        tgt_col = "diff",
                                        ttl=box_title, 
                                        subttl=box_subtitle)
      ...
      plot_dir <- paste0(in_dir, "new_2_", dt_type, "/model_killed/")
