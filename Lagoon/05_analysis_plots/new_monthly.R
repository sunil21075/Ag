...
for (dt_type in in_dir_ext){ # precip or rain or runoff?
  ...
  for (timeP_ty in 1:1){ # annual or chunk or wtr_yr?
    ...
    for (clust_g in cluster_types){
      ...
      ######### Actual value plots
      #########
      AV_box_85 <- Nov_Dec_cum_box(dt = curr_AVs_85, 
                                   y_lab = AV_y_lab, 
                                   tgt_col = AV_tg_col)

      AV_box_45 <- Nov_Dec_cum_box(dt = curr_AVs_45, 
                                   y_lab = AV_y_lab, 
                                   tgt_col = AV_tg_col)
      #########
      ...
      RCP45 <- ggarrange(plotlist = list(AV_box_45, 
                                         unbias_mag_diff_45, unbias_perc_diff_45,
                                           bias_mag_diff_45,   bias_perc_diff_45),
                         ncol = 5, nrow = 1, widths = c(1.5, 1, 1, 1, 1),
                         common.legend = TRUE, 
                         legend="bottom")

      RCP85 <- ggarrange(plotlist = list(AV_box_85, 
                                         unbias_mag_diff_85, unbias_perc_diff_85,
                                           bias_mag_diff_85,   bias_perc_diff_85),
                         ncol = 5, nrow = 1, widths = c(1.5, 1, 1, 1, 1),
                         common.legend = TRUE, 
                         legend="bottom")

      plot_dir <- paste0(in_dir, "new_", dt_type, "/")
      ...
      RCP <- ggarrange(plotlist = list(AV_box_85, 
                                       unbias_mag_diff_85, unbias_perc_diff_85,
                                        bias_mag_diff_85,   bias_perc_diff_85,
                                        AV_box_45, 
                                       unbias_mag_diff_45, unbias_perc_diff_45,
                                        bias_mag_diff_45,   bias_perc_diff_45),
                       ncol = 5, nrow = 2, widths = c(1.5, 1, 1, 1, 1),
                       common.legend = TRUE, 
                       legend="bottom")
      
      plot_dir <- paste0(in_dir, "new_", dt_type, "/both_RCPs")
      if (dir.exists(plot_dir) == F) {dir.create(path = plot_dir, recursive = T)}

      ggsave(filename = paste0(gsub("\ ", "_", clust_g), "_", 
                               timeP_ty_middN[timeP_ty], ".png"),
             plot = RCP, 
