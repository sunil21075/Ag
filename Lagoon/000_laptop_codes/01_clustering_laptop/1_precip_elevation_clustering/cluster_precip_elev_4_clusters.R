...
outputs <- cluster_by_precip_elev(observed, scale=FALSE, no_clusters=4)
clusters <- outputs[[1]]
cluster_obj <- outputs[[2]]
head(clusters, 2)
head(observed, 2)
centes_4_2d_plot <- data.table(cluster_obj$centers)
centes_4_2d_plot$cluster <- 4:1
write.table(centes_4_2d_plot, file = paste0(plot_dir, "four_centroids.csv"),
...
ggsave(filename = "precip_elev_4_clusters.png", 
...
write.table(clusters, 
            file = paste0(out_dir, "precip_elev_4_clusters.csv"),
            row.names = FALSE, na="", 
            col.names=TRUE, sep=",")
