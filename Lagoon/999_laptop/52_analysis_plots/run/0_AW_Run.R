...
for (dt_type in in_dir_ext){
  ...
  for (timeP_ty in length(timeP_ty_middN)){
    ...
    ggsave(filename = paste0(timeP_ty_middN[timeP_ty], "_RCP45.png"),
           plot = AV_box_45,  units = "in",
           width=5.5, height=1.5,
           dpi=600, device = "png", path = just_AVs)

    ggsave(filename = paste0(timeP_ty_middN[timeP_ty], "_RCP85.png"),
           plot = AV_box_85, units = "in",
           width=5.5, height=1.5,
           dpi=600, device = "png", path = just_AVs)
    #####
    ##### Actual Diffs
    #####
    just_diff <- paste0(plot_base, 
                         timeP_ty_middN[timeP_ty], 
                         "/just_diff/")
    ...
    ggsave(filename = paste0(timeP_ty_middN[timeP_ty], "_RCP45.png"),
           plot = unbias_perc_diff_45,  units = "in",
           width=5.5, height=1.5,
           dpi=600, device = "png", path = just_diff)

    ggsave(filename = paste0(timeP_ty_middN[timeP_ty], "_RCP85.png"),
           plot = unbias_perc_diff_85, units = "in",
           width=5.5, height=1.5,
           dpi=600, device = "png", path = just_diff)
