...
for (clust_g in cluster_types){
  ...
  AV_box_85 <- box_trend_monthly_cum(dt = curr_AVs_85, 
                                     ...
                                     y_lab = AV_y_lab, 
                                     tgt_col = AV_tg_col) +
               ggtitle(paste0(AV_title, subttl)) + 
  ...
  #####
  #######################################################
  precip_n_diff_dir <- paste0(plot_base, "precip_n_diff/")
  if (dir.exists(precip_n_diff_dir) == F) {
    dir.create(path = precip_n_diff_dir, recursive = T)}
  print (precip_n_diff_dir)
  precip_n_diff_85 <- ggarrange(plotlist = list(AV_box_85, 
                                        unbias_perc_diff_85),
                       ncol=1, nrow=2, common.legend = TRUE, 
                       legend="bottom")
  
  precip_n_diff_45 <- ggarrange(plotlist = list(AV_box_45, 
                                        unbias_perc_diff_45),
                       ncol=1, nrow=2, common.legend = TRUE, 
                       legend="bottom")
  
  ggsave(filename = paste0(clust_g, "_rain_85.png"),
         plot = precip_n_diff_85, 
         width=8, height=3.5, units = "in", 
         dpi=600, device = "png", path = precip_n_diff_dir)
  
  ggsave(filename = paste0(clust_g, "_rain_45.png"),
         plot = precip_n_diff_45, 
         width=8, height=3.5, units = "in", 
         dpi=600, device = "png", path = precip_n_diff_dir)
  #######################################################
  #####
