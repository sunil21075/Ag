...
main_out <- file.path("/data/hydro/users/Hossein/chill/data_by_core/dynamic_model/02/02_modeled_non_overlap")
...
data_list_2025_2050 <- vector(mode = "list", length = 295)
data_list_2051_2075 <- vector(mode = "list", length = 295)
data_list_2076_2100 <- vector(mode = "list", length = 295)
...
if(hist){
  ...
  for(i in 1:length(the_dir)){
    ...
    # 2025_2050s
    data_list_2025_2050[[i]] <- process_data_non_overlap(file, time_period="2025_2050")
    
    names(data_list_2025_2050)[i] <- the_dir[i]
    
    # 2051_2075s
    data_list_2051_2075[[i]] <- process_data_non_overlap(file, time_period="2051_2075")
    names(data_list_2051_2075)[i] <- the_dir[i]
    
    # 2076_2100s
    data_list_2076_2100[[i]] <- process_data_non_overlap(file, time_period="2076_2100")
    names(data_list_2076_2100)[i] <- the_dir[i]
  ...
  summary_data_2025_2050 <- get_medians(data_list_2025_2050) 
  summary_data_2051_2075 <- get_medians(data_list_2051_2075)
  summary_data_2076_2100 <- get_medians(data_list_2076_2100)
  
  head(summary_data_2025_2050)
  head(summary_data_2051_2075)
  head(summary_data_2076_2100)
  
  # Briefly want to export the raw data from the lists for use in other figs
  data2025_2050 <- ldply(data_list_2025_2050, function(x) data.frame(x))
  data2051_2075 <- ldply(data_list_2051_2075, function(x) data.frame(x))
  data2076_2100 <- ldply(data_list_2076_2100, function(x) data.frame(x))
  
  all_years <- bind_rows(data2025_2050, data2051_2075, data2076_2100)
  ...
  # No longer needed
  rm(list = c("data_list_2025_2050", "data_list_2051_2075", "data_list_2076_2100"))
  ...
  # Grab lat/long
  summary_data_2025_2050 <- grab_coord(summary_data_2025_2050)
  summary_data_2051_2075 <- grab_coord(summary_data_2051_2075)
  summary_data_2076_2100 <- grab_coord(summary_data_2076_2100)
  ...
  summary_data_2025_2050 <- summary_data_2025_2050 %>% mutate(time_period = "2025_2050")
  summary_data_2051_2075 <- summary_data_2051_2075 %>% mutate(time_period = "2051_2075")
  summary_data_2076_2100 <- summary_data_2076_2100 %>% mutate(time_period = "2076_2100")
  
  summary_data_comb <- bind_rows(summary_data_2025_2050,
                                 summary_data_2051_2075,
                                 summary_data_2076_2100)
